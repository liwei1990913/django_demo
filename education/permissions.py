...
from education.permission_list import perm_dic
...
def perm_check(*args,**kwargs):
    ...
    print('---perm:',request.user,request.user,current_url_name)
    #match_flag = False
    match_results = [None,]
    match_key = None
    ...
    for permission_key,permission_val in  perm_dic.items():
        ...
        if per_url_name == current_url_name:
            ...
            if per_method == request.method: #matches request method
                ...
                print(per_method,request.method)
                ...
                if perm_hook_func:
                    perm_hook_matched = perm_hook_func(args[1])
